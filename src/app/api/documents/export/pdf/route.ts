...
export async function GET(request: NextRequest) {
  try {
    ...
    const buffer = await exportToPdf(documents)

    return new NextResponse(buffer, {
      status: 200,
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="ma-tran-tai-lieu_${new Date().toISOString().slice(0, 10)}.pdf"`,
    ...
    return NextResponse.json(
      { error: 'Không thể xuất file PDF. Vui lòng thử lại.' },
      { status: 500 }
    )
  }
}
